...
在对树形结构进行增删改操作时，应遵守以下守则：

#### 7.1 上级节点存在性检查
在 `Create[基础类名]Handler` 和 `Modify[基础类名]BasicHandler` 中，如果 `command.ParentId` 不为 0，必须调用仓储层的 `ExistsAsync` 方法检查该上级节点是否确实存在。

#### 7.2 循环依赖预防
在更新操作中，必须确保新的 `ParentId` 不是当前节点本身，也不是当前节点的任何子孙节点。最基本的校验应包含：
- `command.ParentId != command.Id`

#### 7.3 递归构建树逻辑示例
在 `[基础类名]QueryRepository` 中，推荐使用以下递归模式构建树形结构：

```csharp
public async Task<List<[基础类名]TreeNodeDto>> Get[基础类名]TreeAsync([基础类名]TreeCriteria criteria, CancellationToken cancellationToken)
{
    var query = _db.Set<[基础类名]Po>().AsNoTracking();
    // 应用 Criteria 过滤条件...
    
    var allPos = await query.ToListAsync(cancellationToken);
    var allNodes = allPos.Select(x => new [基础类名]TreeNodeDto { 
        Id = x.Id, 
        ParentId = x.ParentId, 
        Name = x.Name // 对应业务字段
    }).ToList();
...
private List<[基础类名]TreeNodeDto> BuildTree(List<[基础类名]TreeNodeDto> nodes, long parentId)
{
    return nodes.Where(x => x.ParentId == parentId)
                .OrderBy(x => x.Id)
                .Select(x => new [基础类名]TreeNodeDto
                {
                    Id = x.Id,
                    ParentId = x.ParentId,
                    Name = x.Name,
